...
guesses(M) ->
    {I,J,Guesses} = guess(M),
    Ms = lists:map(fun (G) -> catch refine(update_element(M,I,J,G)) end, Guesses),
    SortedGuesses =
	lists:sort(
	  [{hard(NewM),NewM}
	   || NewM <- Ms,
...
solve(M) ->
    ...
    Solution = solve_refined(Refined, ?DEPTH),
...
% 1. spawn a racer, that gets messages Solution/{'EXIT', no_solution}, sends to parent on successful solution
% 2. spawn all guesses, send messages to racer, message parent on solution
% 3. return from parent on message

solve_refined(M, Depth) ->
    case solved(M) of
	true -> M;
	false -> solver(guesses(M), Depth)
    end.
...
solver(Ms, Depth) ->
    Me = self(),
    Racer = spawn(fun () -> racer(Me, length(Ms)) end), % spawn racer
    [spawn(fun () ->
                   Value = (catch solve_refined(M, Depth - 1)),
                   Racer ! Value
           end) || M <- Ms], % spawn guesses
    receive Solution ->
            Solution
    end.

racer(_, 0) -> % tried all guesses
    exit(no_solution);
racer(Parent, Count) ->
    receive M ->
            case M of
                {'EXIT', no_solution} -> % no solution, wait for next solution
                    racer(Parent, Count - 1);
                Solution -> Parent ! Solution % we have a solution!
            end
    end.
